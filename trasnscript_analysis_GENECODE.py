...
# === 1. 加载 iORF 数据 ===
iorf_df = pd.read_csv("V35_iORF.csv")
...
gtf = pd.read_csv("E:/DataBase/gencode.v38.annotation - Copy.gtf", sep="\t", comment="#", header=None, names=gtf_cols, dtype=str)
#gtf = pd.read_csv("test.gtf", sep="\t", comment="#", header=None, names=gtf_cols, dtype=str)
genome = Fasta("E:/DataBase/pri_hg38.fa")


def extract_attr(attr, key):
    match = re.search(f'{key} "([^"]+)"', attr)
    return match.group(1).split(".")[0] if match else None  # 去版本号

gtf["transcript_id"] = gtf["attribute"].apply(lambda x: extract_attr(x, "transcript_id"))
gtf["gene_id"] = gtf["attribute"].apply(lambda x: extract_attr(x, "gene_id"))
gtf["start"] = gtf["start"].astype(int)
gtf["end"] = gtf["end"].astype(int)

# === 3. 构建 transcript 特征：CDS / start_codon / exon / strand ===
transcript_features = defaultdict(lambda: {"CDS": [], "start_codon": [], "stop_codon": [], "exon": [], "strand": None})
gene_to_transcripts = defaultdict(list)

for _, row in gtf.iterrows():
    tid = row["transcript_id"]
    gid = row["gene_id"]
    if pd.isna(tid) or pd.isna(gid):
        continue
    gene_to_transcripts[gid].append(tid)
    if row["feature"] == "CDS":
        transcript_features[tid]["CDS"].append((row["start"], row["end"]))
    elif row["feature"] == "start_codon":
        transcript_features[tid]["start_codon"].append((row["start"], row["end"]))
    elif row["feature"] == "stop_codon":
        transcript_features[tid]["stop_codon"].append((row["start"], row["end"]))
...
for _, row in iorf_df.iterrows():
    ...
    # === 提取该基因所有转录本的 start_codon / stop_codon 区段，统一合并后汇总 ===
    all_start_codons = []
    all_stop_codons = []

    for t in gene_to_transcripts[gid]:
        t_feats = transcript_features.get(t)
        if not t_feats:
            continue
        merged_start = merge_codon_parts_simple(t_feats["start_codon"])
        merged_stop = merge_codon_parts_simple(t_feats["stop_codon"])
        all_start_codons.extend(merged_start)
        all_stop_codons.extend(merged_stop)

    # 去重 + 按 TSS 方向排序
    all_start_codons = sorted(set(all_start_codons), key=lambda x: codon_sort_key(x, iorf_strand))
    all_stop_codons  = sorted(set(all_stop_codons),  key=lambda x: codon_sort_key(x, iorf_strand))

    # 最终将 codon 统一为“转录方向”的 (5' → 3') 显示方式
    ...
    for tid in set(gene_to_transcripts[gid]):
        ...
        start_codons = t_feats["start_codon"]
        stop_codon_sites = t_feats["stop_codon"]
        cds_strand = t_feats["strand"]
        chrom = row["chr"]
        chrom = "chr" + str(chrom)# 提取染色体
...
result_df.to_csv("iORF_transcript_analysis_v38_NEW2.csv", index=False)
